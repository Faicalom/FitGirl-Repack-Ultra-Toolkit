...
def log(message):
    if DEBUG_LOGS or any(marker in message for marker in ["OK", "FAILED", "Retry", "[INFO]", "[WARN]", "[ERROR]", "[SUMMARY]"]):
...
async def process_single_link(context, link, index, filename, total, semaphore, safe_mode=False, fail_list=None):
    async with semaphore:
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            ...
            popup_timeout = SAFE_POPUP_TIMEOUT if is_safe else POPUP_TIMEOUT
            download_timeout = SAFE_DOWNLOAD_TIMEOUT if is_safe else DOWNLOAD_TIMEOUT
            ...
            try:
                ...
                download_btn = await page.wait_for_selector(
                    "text=DOWNLOAD",
                    timeout=BUTTON_TIMEOUT if not is_safe else SAFE_BUTTON_TIMEOUT,
                )
                ...
                log(f"[{index}/{total}] [WARN] Attempt {attempt} failed")
                await page.close()
                ...
                if page:
                    if DEBUG_SCREENSHOTS:
                        try:
                            ...
                            with open(f"debug_failed_{index:03d}.html", "w", encoding="utf-8") as debug_file:
...
async def main():
    ...
    log(f"[INFO] Direct Link Resolver - Concurrency: {CONCURRENCY}")
    ...
    if not links_file:
        log("[ERROR] No source links file found. Run Page_Link_Extractor.py first.")
        return

    index_to_filename, source_to_filename = parse_manifest()

    with open(links_file, "r", encoding="utf-8") as file_obj:
        links = [line.strip() for line in file_obj if line.strip()]
    ...
    async with async_playwright() as playwright:
        ...
        for idx, link in enumerate(links, 1):
            ...
            tasks.append(
                process_single_link(context, link, idx, filename, len(links), semaphore, fail_list=failed_first_pass)
            )

        await asyncio.gather(*tasks)

        if failed_first_pass:
            log(f"[INFO] Starting safe Retry for {len(failed_first_pass)} failed link(s)...")
        ...
        # Write final files in original source order.
        with open("real_direct_links.txt", "w", encoding="utf-8") as file_obj:
            for idx in range(1, len(links) + 1):
                if idx in resolved_map:
                    file_obj.write(resolved_map[idx] + "\n")

        with open("resolved_manifest.txt", "w", encoding="utf-8") as file_obj:
            for idx in range(1, len(links) + 1):
                if idx in resolved_map:
                    filename = index_to_filename.get(idx) or source_to_filename.get(links[idx - 1]) or "unknown"
                    file_obj.write(f"{idx}|{filename}|{resolved_map[idx]}\n")

        with open("resolved_map_debug.txt", "w", encoding="utf-8") as file_obj:
            for idx in range(1, len(links) + 1):
                if idx in resolved_map:
                    filename = index_to_filename.get(idx) or source_to_filename.get(links[idx - 1]) or "unknown"
                    file_obj.write(f"{idx}|{filename}|{links[idx - 1]}|{resolved_map[idx]}\n")

        if final_failed:
            with open("failed_links.txt", "w", encoding="utf-8") as file_obj:
                for idx, link, filename in final_failed:
                    file_obj.write(f"{idx}|{filename}|{link}\n")
            log(f"[WARN] Saved {len(final_failed)} permanently failed link(s) to failed_links.txt")
        else:
        ...
        log("[SUMMARY] Final performance report:")
        log(f"   Total links   : {total}")
        log(f"   Resolved      : {success}")
        log(f"   Failed        : {failed}")
        log(f"   Success rate  : {success_rate:.1f}%")
        log(f"   Total time    : {elapsed:.1f} seconds")
        log("[OK] Stage 2 finished successfully. real_direct_links.txt preserves source order.")
